...
class Meeting(BaseModel):
    ...
    prep_hours_before: int | None = Field(
        None, description="Hours before meeting to trigger prep"
    )

    # References
    chat_session_id: str | None = Field(
        None, description="Associated chat session for prep"
    )
    materials_s3_key: str | None = Field(
        None, description="S3 key for generated materials"
    )
    ...
    def to_dynamodb(self) -> dict[str, Any]:
        ...
        for field in ["start_time", "end_time", "created_at", "updated_at", "last_synced_at", "prep_trigger_time"]:
            if data.get(field):
